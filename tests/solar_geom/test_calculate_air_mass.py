...
@given(
    floats(min_value=0, max_value=90, allow_nan=False, allow_infinity=False),
    floats(min_value=-413, allow_nan=False, allow_infinity=False),
)
def test_calculate_air_mass(zenith, altitude):
    ...
    assert isinstance(
        calculate_air_mass(
            solar_zenith_degrees=zenith, site_altitude_m=altitude
        ),
        float,
    )
...
@pytest.mark.solar_geom
def test_calculate_air_mass_iterable():
    """Functional test to ensure the calculate_air_mass() method
...
def test_known_values():
    ...
    """
    assert calculate_air_mass(
        solar_zenith_degrees=0, site_altitude_m=0
    ) == pytest.approx(0.999709)
    assert calculate_air_mass(
        solar_zenith_degrees=60, site_altitude_m=0
    ) == pytest.approx(1.994244)
...
def test_invalid_type():
    ...
    with pytest.raises(TypeError):
        ...
        assert calculate_air_mass(
            solar_zenith_degrees="blah", site_altitude_m=0
        )
    ...
    with pytest.raises(TypeError):
        # Test with string value
        assert calculate_air_mass(
            solar_zenith_degrees=45, site_altitude_m="blah"
        )
    with pytest.raises(ValueError):
        # Test with NaN value
...
def test_invalid_range():
    ...
    with pytest.raises(ValueError):
        ...
        assert calculate_air_mass(
            solar_zenith_degrees=45, site_altitude_m=-500
        )
