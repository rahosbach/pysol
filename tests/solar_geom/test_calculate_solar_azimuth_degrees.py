...
def test_known_values():
    ...
    assert calculate_solar_azimuth_degrees(
        hour_angle_degrees=-37.5, latitude_degrees=43, declination_degrees=-14
    ) == pytest.approx(-40.08106)
    assert calculate_solar_azimuth_degrees(
...
def test_invalid_type():
    ...
    with pytest.raises(ValueError):
        # Test with NaN value
        assert calculate_solar_azimuth_degrees(
            hour_angle_degrees=0, latitude_degrees=nan, declination_degrees=-14
        )
    with pytest.raises(ValueError):
        # Test with infinite value
        assert calculate_solar_azimuth_degrees(
            hour_angle_degrees=0, latitude_degrees=inf, declination_degrees=-14
        )
    ...
    with pytest.raises(ValueError):
        # Test with NaN value
        assert calculate_solar_azimuth_degrees(
            hour_angle_degrees=0, latitude_degrees=43, declination_degrees=nan
        )
    with pytest.raises(ValueError):
        # Test with infinite value
        assert calculate_solar_azimuth_degrees(
            hour_angle_degrees=0, latitude_degrees=43, declination_degrees=inf
        )
...
def test_invalid_range():
    ...
    with pytest.raises(ValueError):
        assert calculate_solar_azimuth_degrees(
            hour_angle_degrees=-200, latitude_degrees=43, declination_degrees=0
        )
    with pytest.raises(ValueError):
        assert calculate_solar_azimuth_degrees(
            hour_angle_degrees=0, latitude_degrees=-100, declination_degrees=0
        )
    with pytest.raises(ValueError):
        assert calculate_solar_azimuth_degrees(
            hour_angle_degrees=0, latitude_degrees=43, declination_degrees=-30
        )
